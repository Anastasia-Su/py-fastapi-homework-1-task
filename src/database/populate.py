            ...
            async with self._db_session.begin():
                ...
                for _, row in tqdm(
                    ...
                    movie = MovieModel(
                        ...
                        budget=(
                            int(float(budget_value))
                            if budget_value not in (None, "")
                            and not pd.isna(budget_value)
                            else 0
                        ),
                        revenue=(
                            int(float(revenue_value))
                            if revenue_value not in (None, "")
                            and not pd.isna(revenue_value)
                            else 0
                        ),
                        country=row["country"],
                    )
...
async def main() -> None:
    ...
    async with get_db_contextmanager() as db_session:
        if not settings.PATH_TO_MOVIES_CSV:
            raise ValueError("PATH_TO_MOVIES_CSV is not set in settings.")
